...
def viz(folder):
    ghnn = torch.load(folder+"/GHNN.pt").cpu()
    hnn = torch.load(folder+"/HNN.pt").cpu()
    grughnn = torch.load(folder+"/GRUGHNN.pt").cpu()
    list = torch.load(folder+"/evaluation.pt")
    ...
    h = list[2]
    t=list[3]
    #x,dx,h,t = data_load(folder+"fig8")
    #print(dx.shape)
    ...
    xs = [torch.cat((x,dx),dim=-1).squeeze()]
    hs = [h.squeeze().unsqueeze(-1).unsqueeze(-1)]
    ...
    ax[2,3].plot(t,h)
    ax[2,3].plot(t,h_baseline.detach().numpy(),c="g")
    ...
    ax[0,0].plot(x_model[:,0,0].detach().numpy(),x_model[:,0,2].detach().numpy(),c="g")
    ax[0,0].set_title("x and px at body1")
    ax[0,0].legend(["ground truth","prediction"])
    ax[0,0].set_xlabel("x")
    ax[0,0].set_ylabel("px")
    ...
    ax[2,3].plot(t,h_g)
    ax[2,3].plot(t,h_model.detach().numpy(),c="g")
    ax[2,3].set_title("hamiltonian")
    ax[2,3].legend(["ground truth","prediction"])
    ax[2,3].set_xlabel("t")
    ax[2,3].set_ylabel("H")
    ...
    ax[2,3].plot(t,h_g)
    ax[2,3].plot(t,hg.detach().numpy(),c="g")
    ax[2,3].set_title("hamiltonian")
    ax[2,3].legend(["ground truth","prediction"])
    ax[2,3].set_xlabel("t")
    ax[2,3].set_ylabel("H")
    ...
    losses = torch.load(folder+"/losses.pt").cpu()
